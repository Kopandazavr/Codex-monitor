...
public final class ResetCreditActivity extends AppCompatActivity {
    ...
    private void refreshDetailsIfNeeded() {
        ...
        if (SecureTokenStore.isSignedIn(this) && (jMax >= 300000 || missingDetails)) {
            ...
            this.executor.execute(new Runnable() { // from class: dev.kopandazavr.codexmonitor.ResetCreditActivity.4
    ...
    public void consume() {
        ...
        final Context applicationContext = getApplicationContext();
        this.executor.execute(new Runnable() { // from class: dev.kopandazavr.codexmonitor.ResetCreditActivity.6
            @Override // java.lang.Runnable
            public void run() {
                try {
                    ...
                    AppPreferences.setResetCreditsError(applicationContext, ResetCreditActivity.safeMessage(e));
                    ResetCreditActivity.this.runOnUiThread(new Runnable() { // from class: dev.kopandazavr.codexmonitor.ResetCreditActivity.6.2
                        @Override // java.lang.Runnable
                        public void run() {
