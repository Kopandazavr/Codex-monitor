...
grep -q 'protected void onResume()' "$SRC/UsageHistoryActivity.java"
! grep -q 'setZoomEnabled' "$SRC/UsageHistoryActivity.java"
! grep -q 'zoomOut' "$SRC/UsageHistoryActivity.java"
...
grep -q 'Math.abs(dx) > touchSlop && Math.abs(dx) > Math.abs(dy)' "$SRC/UsageBurnChartView.java"
