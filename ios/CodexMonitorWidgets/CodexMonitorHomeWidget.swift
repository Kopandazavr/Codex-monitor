...
struct CodexMonitorHomeWidget: Widget {
    static let kind = "CodexMonitor.Home"
    ...
    var body: some WidgetConfiguration {
        AppIntentConfiguration(
            ...
            CodexMonitorHomeWidgetView(entry: entry)
        }
...
struct CodexMonitorHomeWidgetView: View {
    ...
    private var destination: URL {
        if entry.snapshot.mode == .signedOut {
            return URL(string: "codexmonitor://dashboard")!
        }
        if entry.snapshot.freshness == .empty {
            return URL(string: "codexmonitor://refresh")!
        }
